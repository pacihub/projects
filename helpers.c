...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < h; i++)
    {
        ...
        for (int j = 0; j < w; j++)
        {
            ...
            if (i == 0) // top row
            {
                   ...
                   //i'm using the round function and also dividing by x.0 as the result will be float
                {
                 ...
                 image2[i][j].rgbtGreen = round((image[i][j].rgbtGreen + image[i][j + 1].rgbtGreen + image[i + 1][j].rgbtGreen + image[i + 1][j + 1].rgbtGreen) / 4.0);
                    ...
                    {
            image2[i][j].rgbtRed = round((image[i][w - 1].rgbtRed + image[i][w - 1 - 1].rgbtRed + image[i + 1][w - 1].rgbtRed + image[i + 1][w - 1 - 1].rgbtRed)/4.0);
            image2[i][j].rgbtBlue = round((image[i][w - 1].rgbtBlue + image[i][w - 1 - 1].rgbtBlue + image[i + 1][w - 1].rgbtBlue + image[i + 1][w - 1 - 1].rgbtBlue)/4.0);
            image2[i][j].rgbtGreen = round((image[i][w - 1].rgbtGreen + image[i][w - 1 - 1].rgbtGreen + image[i + 1][w - 1].rgbtGreen + image[i + 1][w - 1 - 1].rgbtGreen)/4.0);
                    }
            ...
            else if (i == h - 1) //bottom row
                {
                    if (j == 0) //bottom row first box
                    {
                        image2[i][j].rgbtRed = round((image[h - 1][j].rgbtRed + image[h - 1][j + 1].rgbtRed + image[h - 1 - 1][j].rgbtRed + image[h - 1 - 1][j + 1].rgbtRed)/4.0);
                        image2[i][j].rgbtBlue = round((image[h - 1][j].rgbtBlue + image[h - 1][j + 1].rgbtBlue + image[h - 1 - 1][j].rgbtBlue + image[h - 1 - 1][j + 1].rgbtBlue)/4.0);
                        image2[i][j].rgbtGreen = round((image[h - 1][j].rgbtGreen + image[h - 1][j + 1].rgbtGreen + image[h - 1 - 1][j].rgbtGreen + image[h - 1 - 1][j + 1].rgbtGreen)/4.0);
                    }
                    else if (j == w - 1) //bottom row last box
                    {
                        image2[i][j].rgbtRed = round((image[i][w - 1].rgbtRed + image[i][w - 1 - 1].rgbtRed + image[i - 1][w - 1].rgbtRed + image[i - 1][w - 1 - 1].rgbtRed)/4.0);
                        image2[i][j].rgbtBlue = round((image[i][w - 1].rgbtBlue + image[i][w - 1 - 1].rgbtBlue + image[i - 1][w - 1].rgbtBlue + image[i - 1][w - 1 - 1].rgbtBlue)/4.0);
                        image2[i][j].rgbtGreen = round((image[i][w - 1].rgbtGreen + image[i][w - 1 - 1].rgbtGreen + image[i - 1][w - 1].rgbtGreen + image[i - 1][w - 1 - 1].rgbtGreen)/4.0);
